...
document.addEventListener('DOMContentLoaded', function () {
  ...
  $(document).ready(function () {
    ...
    $('input[name="paymentMethod"]').change(function () {
      ...
      switch ($(this).attr('id')) {
        case 'bankTransfer':
          $(this).closest('.border-bottom').find('select').prop('disabled', false);
          break;
        case 'eWallet':
          $(this).closest('.border-bottom').find('select').prop('disabled', false);
          break;
        case 'creditCard':
          $('#creditCardForm').collapse('show');
          break;
        case 'qris':
          $('#qrisCode').collapse('show');
          break;
      }
    });
    ...
    $('.btn-success').click(function () {
      ...
      Swal.fire({
        ...
        html: `
        <p>Terima kasih ${name} telah berbelanja di TokoRael</p>
        <p>Detail pesanan akan dikirim ke email: ${email}</p>
        <p>Metode pembayaran: ${selectedPayment}</p>
      `,
        confirmButtonColor: '#28a745',
      }).then((result) => {
        if (result.isConfirmed) {
          // Here you can add redirect logic or other post-order actions
